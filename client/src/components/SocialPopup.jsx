...
const IconInstagram = () => (
  <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
    ...
    <path d="M16 11.37A4 4 0 1 1 12.63 8 4 4 0 0 1 16 11.37z" fill="var(--surface)"/>
    <line x1="17.5" y1="6.5" x2="17.51" y2="6.5" stroke="var(--surface)" strokeWidth="2" strokeLinecap="round"/>
  </svg>
)
const IconLinkedIn = () => (
  <svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor">
...
const SHOW_DELAY_MS = 2500   // 2.5 s after page load
...
export default function SocialPopup() {
  ...
  useEffect(() => {
    // Only show once per browser session (sessionStorage resets on tab close)
    ...
    <>
      ...
      >
        ...
        {[
          ...
          <div key={i} style={{ position: 'absolute', ...s, width: 14, height: 14, borderColor: 'var(--gold)', borderStyle: 'solid' }} />
        ))}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
          ...
          {/* Top — header */}
          <div style={{ padding: '18px 20px 14px', borderBottom: '1px solid var(--border)', background: 'var(--s2)' }}>
            ...
            <h2 style={{ fontFamily: 'Orbitron', fontWeight: 900, fontSize: 'clamp(16px, 3vw, 22px)', color: 'var(--text)', margin: 0 }}>
          ...
          {/* Middle — player card + socials */}
          <div style={{ display: 'flex', gap: 20, padding: '18px 20px', alignItems: 'center', flexWrap: 'wrap' }}>

            {/* Compact player card */}
            <div style={{ flexShrink: 0, transform: 'scale(0.78)', transformOrigin: 'top left', height: 280, width: 202 }}>
              <PlayerCard />
            </div>

            {/* Social links */}
            <div style={{ flex: 1, minWidth: 180 }}>
              <p style={{ fontFamily: 'Barlow Condensed', fontSize: '12px', letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--muted)', marginBottom: 12 }}>
                Find me on
              </p>
      ...
      {/* Popup keyframes injected inline (avoids global CSS coupling) */}
      <style>{`
